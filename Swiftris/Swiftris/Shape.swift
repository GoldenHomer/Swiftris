...
class Shape: Hashable, Printable {
    ...
    final func rotateBlocks(orientation: Orientation) {
        if let blockRowColumnTranslation:Array<(columnDiff: Int, rowDiff: Int)> = blockRowColumnPositions[orientation] {
            ...
            for (index, (columnDiff:Int, rowDiff:Int)) in enumerate(blockRowColumnTranslation) { // enumerate allows interation through an array
    ...
    final func shiftBy(columns: Int, rows: Int) { // shiftBy adjusts each row and column
        self.column += columns
        self.row += rows
        for block in blocks {
